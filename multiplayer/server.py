...
PORT = 12345
connected = False
connecton, address = None, None

# create socket, default IPV4 = AF_INET and TCP + SOCK_STREAM
sock = socket.socket()
# bind socket to host and port
sock.bind((HOST, PORT))
# listening to 1 connection, because two players connected through one connection
sock.listen(1)
...
def receive_data():
    ...
    while True:
        ...
        x = int(data[0])
        y = int(data[1])
        # check if data received indicates server's turn
        if data[2] == 'yourturn':
            turn = True
        ...
        if board.get_cell_val(x, y) == 0:
            board.set_cell_val(x, y, 'O')
        # show data receieved in console
        print(data)

# waits for connection to client
def waiting_to_connect():
    global connected, connection, address
    # this is where it accepts a connection, otherwise the program waits
    connection, address = sock.accept()
    print('Connection Active')
    connected = True
    receive_data()

# runs waiting to connect function in separate thread to prevent game from becoming unresponsive
create_thread(waiting_to_connect)
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and connected:
            if pygame.mouse.get_pressed()[0]:
                # check if turn is active and game isn't over
